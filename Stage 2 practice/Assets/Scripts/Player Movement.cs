...
public class PlayerMovement : MonoBehaviour
{
    ...
    public enum MovementState // 움직임 상태 열거
    {
        walking ,
        running,
        slowlyWalking,
    ...
    void Start()
    {
        rb = GetComponent<Rigidbody>(); // rb는 rigidbody컴포넌트를 갖는다.
        rb.freezeRotation = true; // 리지드바디의 회전값을 제한한다.
    ...
    private void PlayerInput()
    {
        ...
        if(Input.GetKeyDown(crouchKey))
        {
            ...
            rb.AddForce(Vector3.down * 5f, ForceMode.Impulse); // 리지드바디에게 아래쪽 방향으로 순간적인 힘을 5f로 전달해준다. 
        }
    ...
    private void StateHandler()
    {
        ...
        {
            state = MovementState.walking; // 상태가 걸으며 이동하는 상태다.
            moveSpeed = walkSpeed;
        }
    }
    ...
    {
        // 레이캐스트 실행 (경사면에 부딪힌 물체정보 저장)
        if(Physics.Raycast(transform.position, Vector3.down, out slopeHit, playerHeight * 0.5f + 0.3f))
        {
            // 기울기가 얼마만큼 가파른지 계산하기 위한 각도 벡터
            float angle = Vector3.Angle(Vector3.up, slopeHit.normal);
            // 각도가 0이 아닌 maxSlopeAngle보다 작으면 true 반환
